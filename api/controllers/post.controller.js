...
export const getPosts = async (req, res) => {
  ...
  try {
    ...
    if (posts.length === 0) {
      // If no properties are found, send a custom message
      return res.status(404).json({ message: "No properties found in the nearby location" });
    }
...
export const deletePost = async (req, res) => {
  ...
  try {
    ...
    if (post.userId !== tokenUserId) {
      // If not authorized, return early with 403
    ...
    // Only send one response for successful deletion
